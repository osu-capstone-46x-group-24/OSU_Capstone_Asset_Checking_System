import "../App.css";
import Navbar from "../components/UI_Elements/Navbar";
import Footer from "../components/UI_Elements/Footer.tsx";

function Home() {
    return (
        <div className="absolute w-full top-0 left-0 bg-neutral-100 dark:bg-neutral-800">
            <Navbar />

            {/* Body */}
            <div className="relative min-h-180 w-full pt-19">
                <div className="flex flex-col pt-20">
                    <div className="basis-12 py-6 text-7xl font-sans font-size-body mt-20">
                        <a href="https://eecs.engineering.oregonstate.edu/capstone/submission/pages/viewSingleProject.php?id=6RBYZj9tMGMllxTk">
                            CS Capstone
                        </a>
                    </div>
                    <div className="basis-12 py-4 text-3xl text-color-secondary mt-10">
                        <p>OSU CS 46X Capstone Project '25-'26</p>
                    </div>
                </div>
            </div>

            <Footer />
        </div>
    );
}

export default Home;
