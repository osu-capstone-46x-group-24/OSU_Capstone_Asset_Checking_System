import { sqliteTable, AnySQLiteColumn, foreignKey, integer, text, primaryKey, uniqueIndex } from "drizzle-orm/sqlite-core"
  import { sql } from "drizzle-orm"

export const adminsTable = sqliteTable("admins_table", {
	userId: integer("user_id").primaryKey().notNull().references(() => usersTable.id),
});
...
(table) => [
	primaryKey({ columns: [table.userId, table.itemId, table.timestampId], name: "transactions_user_id_item_id_timestamp_id_pk"})
]);
...
(table) => [
	uniqueIndex("users_table_rfid_unique").on(table.rfid),
]);
